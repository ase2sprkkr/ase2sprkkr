...
class TestPotential(TestCase):
    def test_grammar(self):
        ...
        def check(x, prefix=""):
            if "\n" in x.whiteChars:
                raise Exception(
                    f"The potential grammar has newline in whitespace, which is illegal:\n {x}: {prefix}:{x.whiteChars}"
                )
            if hasattr(x, "expr"):
    ...
    def test_reset(self):
        ...
        pot.reset()
        self.assertEqual(2.0, pot.atoms.sites[0].mesh.r1)
        self.assertEqual(2.0, pot.MESH_INFORMATION.DATA()[0][0])
