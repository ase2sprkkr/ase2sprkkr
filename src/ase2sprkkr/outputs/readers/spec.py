...
class SpecResult(TaskResult):
    ...
    def _parse_lattice_info(self) -> None:
        ...
        real_basis_matches = re.finditer(r'real basis\s+\d+\s*:\s*([\d.-]+)\s+([\d.-]+)', content)
        for i, match in enumerate(real_basis_matches, 1):
            self._basis_vectors[f'real_basis_{i}'] = np.array([float(x) for x in match.groups()])
            
        # Parse reciprocal basis vectors
        recip_basis_matches = re.finditer(r'reciprocal basis\s+\d+\s*:\s*([\d.-]+)\s+([\d.-]+)', content)
        for i, match in enumerate(recip_basis_matches, 1):
            self._basis_vectors[f'recip_basis_{i}'] = np.array([float(x) for x in match.groups()])
