...
def k_path_gui(atoms: Atoms, verbose=False):
    ...
    for face in BZ:
        ...
        for i in range(len(coords[:, 0]) - 1):
            x_mid = (coords[i, 0] + coords[i + 1, 0]) / 2.0
            y_mid = (coords[i, 1] + coords[i + 1, 1]) / 2.0
    ...
    if len(selected_path):
        ...
        if verbose:
            ...
            print(
                "The suggested {} k-points along the path corresponds to k-resolution of {} 1/Angstrom.".format(
                    NK, K_resolution
                )
            )
