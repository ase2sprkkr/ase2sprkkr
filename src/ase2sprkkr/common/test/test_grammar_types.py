...
class TestGrammar(TestCase):
    def test_is_the_same_value(self):
        ...
        self.assertTrue(gt.Real.is_the_same_value(1.0, 1.0))
    ...
    def _test_types(self):
        ...
        def test(val, res):
            ...
            self.assertEqual(
                out,
                res,
                "{} should be {} and is {} for type {} after input and output".format(
        ...
        for val, res in [
            ...
            test(val, res)
        for v in [15.0, "aaaa", (1, 2, 3)]:
            test_invalid(v)
        for v, r in [(1, 1.0)]:
            test_warning(v, r)

        type = gt.String()
        for val, res in [("a", "a"), ("a a", Error), ("1", "1")]:
            test(val, res)
        for v in [1, 15.0, (1, 2, 3)]:
            test_invalid(v)
