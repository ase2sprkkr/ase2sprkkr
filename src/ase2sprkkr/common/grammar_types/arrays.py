        ...
        **kwargs,
        ...
        if isinstance(type, (list, np.ndarray)):
            if default_value is not None:
                raise ValueError(
                    "It is not possible for an Array to provide default_value both in 'default_value' and in 'type' argument"
                )
            default_value = type
...
class Table(GrammarType):
    ...
    def __init__(
        self,
        columns: List[GrammarType] = None,
        header: Optional[bool] = None,
        free_header=False,
        format={float: ">22.14", None: ">16"},
        format_all=True,
        numbering: Union[str, bool, GrammarType] = None,
        numbering_label=None,
        numbering_format=True,
        free_numbering: bool = False,
        grouping=False,
        grouping_label=None,
        grouping_format=True,
        prefix=None,
        postfix=None,
        length=None,
        row_condition=None,
            ...
            for i in range(0, len(data), 2):
                ...
                if data[i] != c:
                    raise pp.ParseException(
                        s,
                        loc,
                        "The value in the first column should "
            ...
            for i in range(0, len(data), 3):
                ...
                if data[i + 1] != grp:
                    raise pp.ParseException(
                        s,
                        loc,
                        "The value in the second column should "
    ...
    def _string(self, data):
        ...
        if self.header:
            ...
            formated = (t.format_string(n) for n, t in zip(names, self.sequence.types))
            ...
            for t in types[1:]:
                if t.numpy_type != dtype:
                    if self.named_result is False:
                        raise ValueError(
                            "The table should not have structured dtype, but has columns have different types"
                        )
                    break
            else:
                return dtype
