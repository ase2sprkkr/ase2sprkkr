...
def main():
    ...
    if "msspec" in args:
        ...
        from msspec.utils import SPRKKRPotential, hemispherical_cluster, get_atom_index

        ###########################################################################
        # Build the SrTiO3 cluster                                                #
        ###########################################################################
        # We start by loading the potential since it updates info in the STO cell
        pot = SPRKKRPotential(STO, "SrTiO3_scf.pot_new", *glob.glob("*PHAGEN.pot"))
        ...
        cluster = hemispherical_cluster(STO, planes=4, emitter_plane=1, emitter_tag=1)
        ...
        data = None
        for source_energy in np.arange(500.0, 1501.0, 100.0):
            ...
            dview = dset.add_view(
                "Comparison",
                title=(
                    "Comparison of XPD polar scans for "
                    r"Ti(2p3/2) at $h\nu$ = {:.0f} eV".format(source_energy)
                ),
