...
def make_atoms():
    ...
    atoms.sites = [
        FakeSite(shared_type),
        FakeSite(second_type),
        FakeSite(shared_type),
    ]
    return atoms
...
def test_write_jfile_uses_selector_and_lattice_coordinates():
    rows = np.array(
        [
            (1, 1, 1, 2, 3, 4, 5, 0.1, 0.2, 0.3, 0.75, 6.25, 0.0, 0.0, 0.0),
        ],
        dtype=J_DATA_DTYPE,
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        written = write_jfile(
            output, path, selector=selector, coordinates=Coordinates.lattice
        )
...
def test_write_dmfile_uses_cartesian_coordinates():
    rows = np.array(
        [
            (1, 2, 1, 3, 7, 8, 9, 0.25, 0.5, 0.75, 1.5, 2.5, 3.5, 4.5),
        ],
        dtype=DM_DATA_DTYPE,
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        written = write_dmfile(
            output, path, selector=selector, coordinates=Coordinates.cartesian
        )

        assert written is True
        assert path.read_text() == DMFILE_FORMAT.format(
            2, 3, 0.25, 0.5, 0.75, 2.5, 3.5, 4.5, 1.5
        )
...
def test_write_pos_file_uses_selected_iqs(monkeypatch):
    ...
    monkeypatch.setattr(
        uppasd_bindings.SPRKKRAtoms,
        "promote_ase_atoms",
        staticmethod(lambda atoms: atoms),
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert path.read_text() == (
            POSFILE_FORMAT.format(1, 1, 0.0, 0.1, 0.2)
            + POSFILE_FORMAT.format(3, 1, 2.0, 2.1, 2.2)
        )
...
def test_jxc_output_file_selector_excludes_vacuum_sites(monkeypatch):
    ...
    selector = jxc_output.create_selector(
        it=[1, 2, 3], exclude_it=set(), exclude_vc=True
    )
