...
class NomadApi:
    ...
    def get_authentication_token(self, username, password, expires=None):
        ...
        try:
            ...
            raise ValueError(
                "Nomad response is missing token: \n" + str(response.json())
            )
        except Exception:
            raise RuntimeError(
                "Something went wrong trying to get Nomad authentication token"
            )
    ...
    def create_dataset(self, dataset_name, token=None):
        ...
        try:
            ...
            raise ValueError(
                "Nomad response is missing dataset_id:\n " + str(response.json())
            )
        except Exception as e:
            raise RuntimeError(
                "Something went wrong trying to create a Nomad dataset"
            ) from e
        ...
        def upload():
            try:
                ...
                if upload_id:
                    return upload_id
                raise ValueError(
                    "Nomad response is missing upload_id:\n " + str(response.json())
                )
            except Exception as e:
    ...
    def check_upload_status(self, upload_id, token=None):
        ...
        try:
            ...
            raise ValueError(
                "Nomad response is missing status_message: \n" + str(response.json())
            )
        except Exception as e:
            raise RuntimeError(
    ...
    def edit_upload_metadata(self, upload_id, metadata, token=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Something went wrong trying to add metadata to upload" + upload_id
            ) from e
